...
Throughout the app are examples of a long chain of interactors and controllers, leading to a method that ends up dispatching events to a Store anyway. Simplifying this mental model should allow faster code iteration and code navigation.
...
As demonstrated, there are quite a few abstraction layers until a spot is reach where the Store is being used directly anyway. Following this proposal, the above would change to something like:
...
This drastically reduces the number of layers in the code path, and has the added benefit of clearly capturing additional business logic around the current mode. This details of observing state in the store are complicated in this example by the additional view hierarchies required by recycler views.
